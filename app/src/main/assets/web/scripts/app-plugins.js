/* MesHeures Extensions — V18.0.5 / Lot 6
...
function mhPluginDays(){
  return Object.keys(DB.days||{}).sort().map(k=>{const d=DB.days[k]||{};let r={};try{r=cd(k)||{}}catch(e){}return {date:k,type:d.t||'',start:d.deb||'',end:d.fin||'',pause:d.pause||'',tte:F(r.tte||0),amplitude:F(r.amp||0),alerts:Array.isArray(r.alerts)?r.alerts.length:0}});
}
